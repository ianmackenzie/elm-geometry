...
@docs xInterval, yInterval, zInterval, wInterval, length
...
aggregateOfHelp : Float -> Float -> Float -> Float -> Float -> Float -> Float -> Float -> (a -> VectorBoundingBox4d units coordinates) -> List a -> VectorBoundingBox4d units coordinates
...
-}
length : VectorBoundingBox4d units coordinates -> Interval Float units
length boundingBox =
    let
        ...
        ( Quantity wMin, Quantity wMax ) =
            Interval.endpoints (Interval.abs (wInterval boundingBox))

        minLength =
            Quantity (sqrt (xMin * xMin + yMin * yMin + zMin * zMin + wMin * wMin))

        maxLength =
            Quantity (sqrt (xMax * xMax + yMax * yMax + zMax * zMax + wMax * wMax))
    in
...
-}
randomVector : VectorBoundingBox4d units coordinates -> Generator (Vector4d units coordinates)
randomVector boundingBox =
    let
        parameterValue =
            Random.float 0 1
    in
    Random.map4 (interpolate boundingBox) parameterValue parameterValue parameterValue parameterValue
